...
/*
 * Machine driver for AK4554 on CVITEK CV1835, based on EVAL-adau1372 driver
 *
 * Copyright 2019 CVITEK
 *
 * Author: EthanChen, Eric Brombaugh
 *
 */
...
static const struct snd_soc_dapm_widget cv1835_ak4554_dapm_widgets[] = {
	SND_SOC_DAPM_LINE("Line InL", NULL),
	SND_SOC_DAPM_LINE("Line InR", NULL),
	
	SND_SOC_DAPM_LINE("Line OutL", NULL),
	SND_SOC_DAPM_LINE("Line OutR", NULL),
};

static const struct snd_soc_dapm_route cv1835_ak4554_dapm_routes[] = {
	{ "AINL", NULL, "Line InL" },
	{ "AINR", NULL, "Line InR" },

	{ "Line OutL", NULL, "AOUTL" },
	{ "Line Outr", NULL, "AOUTR" },
};

static int cv1835_ak4554_hw_params(struct snd_pcm_substream *substream,
				     struct snd_pcm_hw_params *params)
{
	return 0;
}

static int cv1835_ak4554_codec_init(struct snd_soc_pcm_runtime *rtd)
{
	/* just use defaults */
	u32 junk;	// just to generate a warning so I know it compiled
#if 0
...
static struct snd_soc_ops cv1835_ak4554_ops = {
	.hw_params = cv1835_ak4554_hw_params,
};

static struct snd_soc_dai_link cv1835_ak4554_dai[] = {
	{
		.ops = &cv1835_ak4554_ops,
		.init = cv1835_ak4554_codec_init,
	},
...
static struct snd_soc_card cv1835_ak4554 = {
	.owner = THIS_MODULE,
	.dai_link = cv1835_ak4554_dai,
	.num_links = ARRAY_SIZE(cv1835_ak4554_dai),

	.dapm_widgets		= cv1835_ak4554_dapm_widgets,
	.num_dapm_widgets	= ARRAY_SIZE(cv1835_ak4554_dapm_widgets),
	.dapm_routes		= cv1835_ak4554_dapm_routes,
	.num_dapm_routes	= ARRAY_SIZE(cv1835_ak4554_dapm_routes),
};

static const struct of_device_id cvi_audio_match_ids[] = {
	{
		.compatible = "cvitek,cv1835-ak4554",
		//.data = (void *) &cv1835_ak4554_dai,
	},
	{ }
};
...
static int cv1835_ak4554_probe(struct platform_device *pdev)
{
...
static struct platform_driver cv1835_ak4554_driver = {
	.driver = {
		.name = "cv1835-ak4554",
		.pm = &snd_soc_pm_ops,
		.of_match_table = cvi_audio_match_ids,
	},
	.probe = cv1835_ak4554_probe,
};

module_platform_driver(cv1835_ak4554_driver);

MODULE_AUTHOR("EthanChen");
MODULE_DESCRIPTION("ALSA SoC cv1835 ak4554 driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:cv1835-ak4554");
